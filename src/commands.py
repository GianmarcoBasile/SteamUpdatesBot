...
async def start(update, context):
    ...
    if not mongo_instance['USERS'][sender].find_one({'chat_id': update.message.chat_id}):
        ...
        mongo_instance['GAMES']['games'].insert_one({'user': sender, 'games': {}, 'last_news':{}})
...
async def getNews(update, context):
    if not context.args:
        ...
        for game in games['games'].values():
            r = requests.get('http://api.steampowered.com/ISteamNews/GetNewsForApp/v0002/?appid=' + str(get_game_id_by_name(game)) + '&count=10&maxlength=50000&format=json')
